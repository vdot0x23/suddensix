...
ROUTERLOG="/var/log/fake_router26.log"
...
#Install packages, these should all be in the standard Kali repos
function installPrereqDpkgs {
    /usr/bin/apt-get install -y sipcalc tayga radvd wide-dhcpv6-server bind9 iptables thc-ipv6
}
...
function startFakeRouter {
	...
	atk6-fake_router26 ${DINTERFACE} -E H1111 -A ${DEFAULT6PREFIX}:/${DIP6CIDR} -F other -D ${DIP6} -d 30
	sleep 3
    done
}
